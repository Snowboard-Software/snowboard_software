...
Dot usage is measured in **Agent Compute Credits (ACCs)**. An ACC reflects the amount of work Dot does to answer a request: the queries it runs, the reasoning steps it takes, and the visualizations it builds. A quick lookup uses very little; a deep, multi-step [investigation](analyze.md) uses more — and a higher [energy mode](energy-modes.md) does more work per question.
